...
export default async function CreadorDashboardPage() {
    ...
    const isCreatorOrAdmin = isCreator(session.user.role)

    const courses = await prisma.course.findMany({
        where: isCreatorOrAdmin ? {} : { instructorId: session.user.id },
        include: {
            instructor: {
                select: {
                    id: true,
    ...
    return (
        <div className="p-6">
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {courses.map((course) => (
                    <Link key={course.id} href={`/creador/courses/${course.id}`}>
                        <Card className="hover:shadow-lg transition-shadow cursor-pointer h-full">
                            <CardHeader>
                                <CardTitle className="line-clamp-1">{course.title}</CardTitle>
                                {isCreatorOrAdmin && course.instructor && (
